...
use crate::{RlnPublicInputs, RlnWitness, RlnGroth16Prover, RlnGroth16Verifier, RlnSetup};
#[cfg(feature = "arkworks")]
use ark_bn254::Fr;
#[cfg(feature = "arkworks")]
...
impl RlnProver {
    ...
    fn prove_with_groth16(
        ...
        let witness = RlnWitness {
            secret: secret_field,
            merkle_proof: proof_hashes.iter().map(|h| bytes_to_field(h)).collect(),
...
impl RlnVerifier {
    ...
    pub fn verify(&mut self, proof: &RlnProof) -> Result<()> {
        ...
        let has_duplicate = self.seen_nullifiers
            .get(&proof.epoch)
    ...
    #[cfg(feature = "arkworks")]
    fn verify_with_groth16(
        &self,
        verifier: &RlnGroth16Verifier,
        proof: &RlnProof,
    ) -> Result<()> {
        ...
        let public_inputs = RlnPublicInputs {
            root: merkle_root,
            nullifier: bytes_to_field(&proof.nullifier.as_slice()[..32.min(proof.nullifier.len())].try_into().unwrap_or([0u8; 32])),
        ...
        if !valid {
            return Err(ZkError::InvalidProof("Groth16 verification failed".to_string()));
        }
    ...
    pub fn cleanup_old_nullifiers(&mut self, current_epoch: u64, keep_epochs: u64) {
        self.seen_nullifiers.retain(|&epoch, _| {
            epoch >= current_epoch.saturating_sub(keep_epochs)
        });
    }
}
