...
- Sealed local state: encrypted vault + RAM-only mode
- Minimal metadata: no server logs, no social graph DB
...
- Optional USB-backed encrypted state (device-bound ML-KEM key)
- Vanish-on-close: RAM-only mode leaves no disk artifacts
...
| Social graph mapping | Reveal who-talks-to-who | Onion routing, no server metadata |
...
| Stolen device → key exfil | Read past messages | Forward secrecy (epoch rekey), optional sealed vault |
