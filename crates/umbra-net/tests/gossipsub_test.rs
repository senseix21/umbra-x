...
async fn test_gossipsub_message_exchange() {
    ...
    // Create node1
    let mut node1 = P2PNode::new_with_port(29001).await.unwrap();
    ...
    node1.subscribe("umbra-test").unwrap();
    
    // Create node2
    let mut node2 = P2PNode::new_with_port(29002).await.unwrap();
    ...
    let t2 = tokio::spawn(async move {
        ...
        let _ = node2.publish("umbra-test", b"Hello UMBRA!".to_vec());
